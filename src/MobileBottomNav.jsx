        ...
        {NAV.map((item) => (
          <NavLink key={item.to} to={item.to} end={item.to === "/"}
            style={({ isActive }) => ({
              ...
              flex: 1, gap: 3, padding: "4px 2px",
              color: isActive ? "var(--accent-light)" : "var(--muted)",
              textDecoration: "none", fontSize: 10,
              fontWeight: isActive ? 700 : 400, transition: "color 0.12s",
              position: "relative",
            })}
          >
            {({ isActive }) => (
              <>
                {isActive && (
                  <div style={{ position: "absolute", top: 0, left: "50%", transform: "translateX(-50%)", width: 24, height: 2, borderRadius: 2, background: "var(--accent)" }} />
                )}
