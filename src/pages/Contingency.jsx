...
// Aba de Contingência — versão com badges de copiar + layout responsivo mobile
// Campo de email removido. Cards em mobile, tabela em desktop.
// ─────────────────────────────────────────────────────────────────────────────
...
function exportCSV(accounts) {
  ...
  const lines   = [
    headers.join(","),
    ...accounts.map((a) =>
      [a.username, a.senha, a.token2fa, a.nome,
...
function CopyBadge({ text, title = "Copiar", disabled = false }) {
...
function FieldWithCopy({ value, onChange, placeholder, type = "text", monospace = false, copyTitle }) {
  return (
      ...
      {expanded && (
        <div style={{ padding: "0 14px 14px", borderTop: "1px solid var(--border)" }}>
          ...
          <div style={{ marginTop: 10 }}>
            ...
            <FieldWithCopy value={acc.token2fa} onChange={(v) => handleDebounced("token2fa", v)} placeholder="token2fa" monospace copyTitle="Copiar token 2FA" />
          </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 5, flex: 1, minWidth: 0 }}>
            <input
              ...
              style={{ fontSize: 12, padding: "4px 8px", fontWeight: 600, flex: 1, minWidth: 80 }}
            />
      ...
      <td style={{ padding: "10px 12px", background: rowBg }}>
        <div style={{ display: "flex", gap: 5, alignItems: "center" }}>
          <input
            ...
            style={{ fontSize: 11, padding: "4px 8px", fontFamily: "monospace", flex: 1, minWidth: 100 }}
          />
          <CopyBadge text={acc.token2fa} title="Copiar token 2FA" />
        </div>
      </td>
      <td style={{ padding: "10px 12px", background: rowBg }}>
        <StatusSelect value={acc.status} onChange={(v) => handleImmediate("status", v)} />
...
export default function Contingency() {
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      ...
      const created = await Promise.all(
        rows.map((row) =>
          saveAccount({
            id: uid(), username: row.username || "",
            senha: row.senha || "", token2fa: row.token2fa || "",
            nome: row.nome || "", status: "preparada", qualidade: "boa",
  ...
  const handleMoveToMain = useCallback(async () => {
    showToast("error",
  ...
  const handleAddEmpty = useCallback(async () => {
    const now = new Date().toISOString();
    await saveAccount({
      id: uid(), username: "", senha: "", token2fa: "",
      nome: "", status: "preparada", qualidade: "boa",
      notas: "", created_at: now, updated_at: now,
    });
    ...
    <div style={{ padding: isMobile ? "16px 12px" : "24px 28px", maxWidth: 1400, margin: "0 auto" }}>
      ...
      <div style={{ marginBottom: 20 }}>
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
          <div>
            <h1 style={{ fontSize: isMobile ? 18 : 22, fontWeight: 800, color: "var(--text)", marginBottom: 4, display: "flex", alignItems: "center", gap: 8 }}>
              🛡️ Contingência
            </h1>
            {!isMobile && (
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
            <button className="btn btn-ghost btn-sm" onClick={handleAddEmpty}>
              {isMobile ? "➕" : "➕ Adicionar"}
            </button>
            ...
            <button className="btn btn-ghost btn-sm" onClick={handleExport} disabled={filtered.length === 0}>
        ...
        {STATUS_OPTIONS.map((s) => (
          <button
            ...
            style={{
              ...
              cursor: "pointer", textAlign: "center", minWidth: 80, flex: "0 0 auto",
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 14, flexWrap: "wrap", alignItems: "center" }}>
        ...
        {(search || filterStatus !== "todas") && (
          <button className="btn btn-ghost btn-sm" onClick={() => { setSearch(""); setFilterStatus("todas"); }}>
            ✕ {!isMobile && "Limpar"}
          </button>
        )}
      ...
      {accounts.length === 0 && !loading && (
        <div style={{
          ...
          borderRadius: "var(--radius)", padding: "36px 24px", textAlign: "center", marginBottom: 20,
        }}>
          ...
          <div style={{ fontSize: 12, color: "var(--muted)", marginBottom: 18, lineHeight: 1.7 }}>
            Importe um CSV com as colunas:{" "}
            <code style={{ color: "var(--accent3)", background: "var(--bg3)", padding: "2px 6px", borderRadius: 4 }}>username, senha, token2fa, nome</code>
            <br />ou adicione manualmente com <strong>➕ Adicionar</strong>.
          </div>
      ...
      {/* Conteúdo: Cards (mobile) ou Tabela (desktop) */}
      {filtered.length > 0 && (
        isMobile ? (
          <div>
            {filtered.map((acc) => (
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
              <thead>
                <tr style={{ background: "var(--bg2)", borderBottom: "1px solid var(--border2)" }}>
                  {["Username (@)", "Senha", "Token 2FA", "Status", "Qualidade", "Notas", "Atualizado em", "Ações"].map((h) => (
                    <th key={h} style={{
      ...
      {accounts.length > 0 && (
        <div style={{ marginTop: 14, fontSize: 10, color: "var(--muted)", textAlign: "right" }}>
          💾 Dados salvos localmente no navegador (IndexedDB). Exportar CSV para backup.
        </div>
      )}
